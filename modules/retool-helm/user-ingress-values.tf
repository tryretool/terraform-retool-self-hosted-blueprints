locals {
  ...
  user_ingress_values = var.user_ingress == null ? [yamlencode(local.null_user_ingress_values)] : [
    yamlencode(merge(
      ...
      var.user_ingress.ingress_mode == "external" ? local.null_user_ingress_values : {},

      # "gke-gateway-controller": disable the chart Ingress and attach a Gateway
      # API HTTPRoute to the user-ingress Gateway.
      var.user_ingress.ingress_mode == "gke-gateway-controller" ? {
        ingress = {
          enabled = false
        }
        httpRoute = {
      ...
      # secret (HTTPS path).
      var.user_ingress.ingress_mode == "azure-agic" ? {
        ingress = {
          enabled          = true
      ...
      var.user_ingress.ingress_mode == "azure-agic" && var.user_ingress.agent_sandbox_proxy_enabled && var.domain_name != null ? {
        agentSandbox = {
          proxy = {
            ingress = {
              enabled          = true
