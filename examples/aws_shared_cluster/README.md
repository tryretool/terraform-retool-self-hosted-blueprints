# AWS — two Retool instances in one EKS cluster
...
This example is that guidance written out for AWS.

It stands up one VPC and one EKS cluster, then deploys two independent Retool
...
2. In `shared.tf`, set `prefix_global` and `region`.
